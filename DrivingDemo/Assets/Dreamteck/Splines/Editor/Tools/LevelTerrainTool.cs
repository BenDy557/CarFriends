...
namespace Dreamteck.Splines
{
    public class LevelTerrainTool : SplineTool
    {
        ...
        {
            if(computers.Count == 0) GetSplines();
        ...
        public override void Draw(Rect windowRect)
        {
            ...
            if (computers.Count == 0) EditorGUILayout.HelpBox("No spline selected! Select an object with a SplineComputer component.", MessageType.Warning);
            if (terrain == null) EditorGUILayout.HelpBox("No terrain selected! You need to select a terrain.", MessageType.Warning);
            if (computers.Count == 0 || terrain == null) return;
            if (!init)
            {
                init = true;
        ...
        void CarveTerrain()
        {
            ...
            {
                PaintHeightMap(terrain, computers[i], ref drawLayer, ref alphaLayer);
            }
